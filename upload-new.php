...
if (isset($_POST['submit']))
{
	...
	fclose($fp);
	$sql="Insert into `DocStorageTable` (`name`,`path`,`upload_by`,`upload_date`,`status`,`file_type`,`content`) values ('$fileName','$path','$uploadBy','$uploadDate','active','$docType')";
	$dblink->query($sql) or
	...
	fwrite($fp,$content);
	fclose($fp);
	header("Location: http://192.168.56.103/upload.php?msg=success");
}
?>